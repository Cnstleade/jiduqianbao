<template>
	<div class="container">
		<el-row>
			<el-alert title="审核分配" :closable="false" type="info">
			</el-alert>
		</el-row>
		<el-tabs type="border-card" v-model="activename" class="m20" @tab-click="handleClick">
			<el-row class="m20">
				<el-col class="col-flex-end">
					...
					<div class="l20">
						<el-input style="padding:0px 10px 0px 0px" placeholder="请输入姓名" v-model="search.username" clearable>
						</el-input>
					</div>
					<div class="l20">
						<el-input style="padding:0px 10px 0px 0px" placeholder="请输入身份证" v-model="search.idcard" clearable>
						</el-input>
					</div>
					<div class="l20">
						<el-input style="padding:0px 10px 0px 0px" placeholder="请输入手机号码" v-model="search.phone" clearable>
						</el-input>
					</div>
					<el-date-picker style="width:340px" class="l20" v-model="search.time" type="daterange" value-format="yyyy-MM-dd"
			...
			<el-tab-pane label="待通过" name="second"></el-tab-pane>
			<el-tab-pane label="已分配订单" name="three"></el-tab-pane>

			<el-table v-loading="loading" :data="tableData" class="textContent m20" ref="multipleTable" @selection-change="handleSelectionChange"
			 highlight-current-row style="width: 100%;font-weight:bold" border>
				<el-table-column type="selection" v-if="activename=='first'" width="55">
				</el-table-column>
				...
				<el-table-column label="审核人" align="center">
					<template slot-scope="scope">
						<div v-if="scope.row.operatorName === ''|| scope.row.operatorName === null">
							<el-button type="info" size="mini" @click="allotOperator(scope.row,false)">分配</el-button>
						</div>
						<span v-else>
				...
				<el-table-column label="贷款状态" align="center" width="100">
					<template slot-scope="scope">
						<span>
              ...
              :scope.row.status===7?'还款':'完结'}}
				...
				<el-table-column prop="applyAmt" label="授信额度" align="center" width="80"></el-table-column>
				...
				<el-table-column label="审核结果" align="center" width="140">
					<template slot-scope="scope">
						<el-tag :type="scope.row.approveResult ===1?''
			...
			<el-row class="m20" v-if="total>0">
				<el-button type="primary" style="float:left" @click="allotOperator('',true)" v-if="activename=='first'" :disabled="multipleSelection.length==0">批量分配</el-button>
				<div style="float:right">
					<el-pagination @current-change="handleCurrentChange" @size-change="handleSizeChange" :current-page="npage"
					 :page-sizes="[10, 20, 50, 100, 1000,990000]" :page-size="pagesize" background layout="total,sizes,prev, pager, next,jumper"
					 :total="total">
					</el-pagination>
				</div>
			</el-row>
		...
		<el-dialog title="分配客户" :visible.sync="dialogAssessor" width="30%" center>
			<el-form class="m20" :model="fbshyForm" status-icon ref="fbshyForm" label-width="150px" :rules="rules">
				<el-form-item label="请选择审核员" prop="assessor">
					<el-select v-model="fbshyForm.assessor" placeholder="请选择审核员">
						<template v-for="item in assessorList">
							<el-option :key="item.uid" :label="item.username" :value="item.uid"></el-option>
						</template>
					</el-select>
				</el-form-item>
			</el-form>
			<div slot="footer" class="dialog-footer">
				...
				<el-button type="primary" @click="submitForm('fbshyForm')">确 定</el-button>
			</div>
		</el-dialog>
		<el-dialog :visible.sync="outerVisible" width="90%" title="客户信息详情" center id="text">
			...
			<el-row style="overflow: hidden;">
				<table class="table textContent">
					...
					<tr>
						...
						<td colspan="5">
							<el-table :data="customerInformation" border highlight-current-row style="width: 100%;font-weight:bold">
								...
								<el-table-column prop="chbstatus" align="center" label="状态" width="80">
									<template slot-scope="scope">
										<el-tag :type="scope.row.chbstatus===1?'success':scope.row.chbstatus===2?'danger':''">
								...
								<el-table-column prop="chbisDel" align="center" label="是否黑名单" width="100">
									<template slot-scope="scope">
										<el-tag :type="scope.row.chbisDel?'danger':'success'">{{scope.row.chbisDel?'是':'否'}}</el-tag>
									</template>
								</el-table-column>
              ...
              overflow: hidden;"
								...
								<el-table-column prop="source" align="center" label="来源" width="80">
									<template slot-scope="scope">
										<el-tag :type="scope.row.status===1?'':scope.row.status===2?'success':'danger'">{{scope.row.status===1?'ios':scope.row.status===2?'android':'h5'}}</el-tag>
									</template>
								</el-table-column>
							</el-table>
						</td>
					...
					<tr>
						...
						<td colspan="5">
							<el-table v-if="customerInformation[0]" :data="customerInformation[0].bankCardDTOVoList" border
					...
					<tr>
						...
						<td colspan="5">
							<template v-if='ddShow&& customerInformation[0]'>
								<el-table :data="customerInformation[0].didiOrderVoList.slice(0,5)" class="textcontent" border
								 highlight-current-row style="width: 100%;font-weight:bold">
							...
							<template v-if='!ddShow&& customerInformation[0]'>
								<el-table :data="customerInformation[0].didiOrderVoList" border highlight-current-row style="width: 100%;font-weight:bold">
									<el-table-column prop="cityName" fixed align="center" label="城市" width="100"></el-table-column>
									<el-table-column prop="productName" align="center" label="类别" width="100"></el-table-column>
									<el-table-column prop="actualPayFee" align="center" label="消费记录" width="120"></el-table-column>
									<el-table-column prop="fromAddress" align="center" label="起始地" min-width="200">
					...
					<tr>
						...
						<td colspan="5">
							<el-table v-if='customerInformation[0]' :data="customerInformation[0].chbcredit" border highlight-current-row
					...
					<tr>
						...
						<td colspan="5">
							<el-table v-if="customerInformation[0]" :data="customerInformation[0].remarkList" border highlight-current-row
							 style="width: 100%;font-weight:bold">
								<el-table-column prop="createTime" align="center" label="备注时间" fixed width="180">
									<template slot-scope="scope">
										{{scope.row.createTime|dateServer}}
									</template>
								</el-table-column>
								<el-table-column prop="userPosition" align="center" label="职位" width="180">
									<template slot-scope="scope">
										{{scope.row.userPosition==1?"审核员":'审核经理'}}
									</template>
								</el-table-column>
								<el-table-column prop="type" align="center" width="180" label="类型">
									<template slot-scope="scope">
										{{scope.row.type==1?"审核员驳回":scope.row.type==2?"审核人员拒绝":scope.row.type==3?"审核经理驳回":scope.row.type==4?"审核经理拒绝":'通过'}}
									</template>
								</el-table-column>
								<el-table-column prop="userName" align="center" width="180" label="姓名">
								</el-table-column>
								<el-table-column prop="remake" align="center" min-width="180" label="备注">
									<template slot-scope="scope">
			...
			<el-dialog id="dialog" width="80%" top="100px" title="照片信息" :visible.sync="innerImgVisible" append-to-body>
			...
			<el-dialog width="30%" title="通讯录" :visible.sync="contactList" append-to-body>
			...
			<el-dialog id="report" width="70%" title="风控报告" :visible.sync="CreditReport" append-to-body>
					...
					<el-tab-pane label="同盾信用报告" name="second">
						<el-row v-if="JSON.stringify(baseInfo) !== '{}'">
                            ...
                            2?'建议人工审核':'建议通过')}}</p>
											...
											<el-progress type="circle" :percentage="baseInfo.baseInfo.score>100 ?99 : baseInfo.baseInfo.score" color="red"
			...
			<el-dialog title="审核结果" :visible.sync="dialogResult" append-to-body width="30%" center>
				<el-row class="col-flex">
					<el-button type="success">最低授信额度{{chooseMoneyVo.minMoney}}</el-button>
					<el-button class="l20" type="success">最高授信额度{{chooseMoneyVo.maxMoney}}</el-button>
		...
		methods: {
			getData(
				...
				let url =
					...
					httpSysAllocatedList :
					this.activename == "second" ? httpManagerReview : httpAssignedList;
				url(
						loginId,
						npage,
						pagesize,
			...
			//提交审核员
			_httpSetReviewe(loginId, id, Userid) {
				httpSetReviewe(loginId, id, Userid)
					.then(res => {
						let data = res.data;
			...
			submitForm(formName) {
				this.$refs[formName].validate(valid => {
					if (valid) {
						if (formName == "fbshyForm") {
							this._httpSetReviewe(
								this.loginId,
								this.rowId,
								this.fbshyForm.assessor
							);
			...
			addResult() {
				...
				this.resultForm.age =
					...
					this.customerInformation[0].approveAmt :
					"";
			},
			clickImg(e, src) {
